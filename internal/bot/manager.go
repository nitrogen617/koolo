...
type SupervisorManager struct {
	...
	delivery *delivery.Service	// DELIVERY: Service façade to manage delivery domain
}
...
func NewSupervisorManager(logger *slog.Logger, eventListener *event.Listener) *SupervisorManager {
	...
	return &SupervisorManager{
		...
		delivery:       delivery.NewService(logger),
	}
}
...
func (mng *SupervisorManager) buildSupervisor(supervisorName string, logger *slog.Logger, attach bool, optionalPID uint32, optionalHWND win.HWND) (Supervisor, *game.CrashDetector, error) {
	...
	// DELIVERY: Attach delivery manager to delivery service (filters, callbacks, queued requests)
	if mng.delivery != nil {
		if ctx.Delivery == nil {
			ctx.Delivery = delivery.NewManager(supervisorName, logger)
		}
		mng.delivery.AttachManager(supervisorName, ctx.Delivery)
	}
...
// DELIVERY: Expose delivery service for server wiring
func (mng *SupervisorManager) DeliveryService() *delivery.Service {
	return mng.delivery
}
